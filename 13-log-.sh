...
# Creates the logs directory if it doesn't exist. The -p flag ensures that
# parent directories are also created if needed, and suppresses errors if
# the directory already exists. This is useful for initializing the logging
# folder structure before writing log files to the $LOGS_FOLDER location.
mkdir -p $LOGS_FOLDER
